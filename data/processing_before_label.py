...
def create_imgs_and_mask(satpath, list_output_path, list_of_fn):
    """
    Function create RGB, RGBN images and draft of mask from raw data
    downloded from satellite. With images also do simple correction
    of brightness, gamma and normalize values of imgs
    """

    def save_img(output_path, output_filename, list_of_bands, band_orig):
        img = rio.open(
    ...
    def simple_transformation(band):
        # brightness
        alpha = 0.09
        beta = 0.4
    ...
    # read blue
    band2 = rio.open(
        satpath + "T37VCD_20220703T084611_B02_10m.jp2", driver="JP2OpenJPEG"
    )
    # read green
    band3 = rio.open(
        satpath + "T37VCD_20220703T084611_B03_10m.jp2", driver="JP2OpenJPEG"
    )
    # read red
    band4 = rio.open(
        satpath + "T37VCD_20220703T084611_B04_10m.jp2", driver="JP2OpenJPEG"
    )
    # read nir
    band8 = rio.open(
        satpath + "T37VCD_20220703T084611_B08_10m.jp2", driver="JP2OpenJPEG"
    )
    ...
    list_bands_rgbn = [simple_transformation(band) for band in list_bands_rgb]
    # mask is black, that way no need to apply func above
    list_bands_mask = [mask]

    # saving RGB, RGBN and mask
    # IMPORTANT need to give original band, because it exist crs
